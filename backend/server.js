...
// ==========================================
// --- API NHẬP EXCEL THEO TỪNG MODULE ---
// ==========================================
...
app.post('/api/import/items', authenticate, requireRole(['admin', 'ktt']), upload.single('file'), async (req, res) => {
  try {
...
app.post('/api/import/opening-balances', authenticate, requireRole(['admin', 'ktt']), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'Vui lòng chọn file Excel để nhập.' });
...
app.post('/api/import/vouchers', authenticate, requireRole(['admin', 'ktt']), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'Vui lòng chọn file Excel để nhập.' });
