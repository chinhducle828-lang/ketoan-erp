  ...
  try {
    ...
    for (const d of details) {
      ...
      if (BANNED_ACCOUNTS_TT99.includes(mainAccount)) {
        return res.status(400).json({ error: `Tài khoản ${d.accountCode} không còn hiệu lực hạch toán từ kỳ 2026 (TT 99).` });
      }

      // Nghiệp vụ mua hàng phải ghi Có tài khoản công nợ phải trả (331), tiền mặt (111) hoặc tiền gửi ngân hàng (112)
